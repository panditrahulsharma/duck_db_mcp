...
def ensure_database() -> None:
    """Create and seed the DuckDB database if it is missing or empty."""
    ...
    if db_path.exists() and db_path.stat().st_size > 0:
        ...
        try:
            ...
            if any(table[0] in {"employee", "department", "salary", "attendance"} for table in tables):
                conn.close()
                return
        finally:
            conn.close()
    ...
    if not STATEMENTS:
        raise RuntimeError("Seed statements are not available. Run seed_employee_db.py manually.")
